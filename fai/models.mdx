...
<u>[i don't think thinking_budget is consistent across all providers AND we should have some ability to pre_fill tokens - to do more research AND we need to test if extra_headers work. there needs to be more explanatinos into the inputs and why.]</u>
...
Once these are avaialble, you need to ensure that in your environment that you have either exported the necessary envs (see [quickstart](/fai/quickstart)) or added `.env` to your `/<instance-name.`The API keys you require depends on the providers e.g. if `provider=google`, then you'll need `GOOGLE_API_KEY`
